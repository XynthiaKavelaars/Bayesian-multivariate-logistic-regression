...
ComputePowerComp <- function(phiE, phiC, weights, n, alpha=0.05, one.sided = TRUE){
  ...
  z=(muE-muC)/sqrt((varE+varC)/n)
 
  if(one.sided & muE - muC > 1e-2){
    pwr <- pnorm(abs(z)-qnorm(1-alpha))
  } else if (!one.sided & abs(muE - muC) > 1e-2){
    pwr <- pnorm(abs(z)-qnorm(1-alpha/2))
  }
  return(pwr)
}
...
# Output: 
# pwr: Probability to reject H0

ComputePowerAny <- function(phiE, phiC, n, alpha=0.05, one.sided = TRUE){
  indicesTheta <- IndicesTheta(length(phiE))
  thetaE <- Phi2Theta(phiE, Indices = indicesTheta, K = length(indicesTheta))
  thetaC <- Phi2Theta(phiC, Indices = indicesTheta, K = length(indicesTheta))
  ...
  delta <- thetaE - thetaC
    ...
    if(one.sided){ 
      ...
      pwr <- 1-mvtnorm::pmvnorm(lower=-Inf, upper=z.r, corr=Sigma)
      
    } else if (!one.sided ){
      z.l <- c(qnorm(alpha/(length(theta)*2)) - delta / se)
      z.r <- c(qnorm(1 - alpha/(length(theta)*2)) - delta / se)
      
      pwr <- 1-mvtnorm::pmvnorm(lower=-Inf, upper=z.r, corr=Sigma) + 1-mvtnorm::pmvnorm(lower=z.l, upper=Inf, corr=Sigma)
...
ComputePowerAll <- function(phiE, phiC, n, alpha=0.05, one.sided = TRUE){
  indicesTheta <- IndicesTheta(length(phiE))
  thetaE <- Phi2Theta(phiE, Indices = indicesTheta, K = length(indicesTheta))
  thetaC <- Phi2Theta(phiC, Indices = indicesTheta, K = length(indicesTheta))
  ...
  delta <- thetaE - thetaC
    ...
    if(one.sided){ 
      ...
      pwr <- mvtnorm::pmvnorm(lower=-Inf, upper = z.r, 0, corr=Sigma) 
    } else if (!one.sided ){
      z.r <- c(delta / se - qnorm(1 - alpha/2))
      z.l <- c(delta / se - qnorm(alpha/2))
      
      pwr <- mvtnorm::pmvnorm(lower=-Inf, upper = z.r, mean = 0, corr=Sigma) + mvtnorm::pmvnorm(lower = z.l, upper = Inf, mean = 0, corr = Sigma)
    }
